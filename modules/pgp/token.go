...
func GenerateToken(signingKey *rsa.PrivateKey, message string) (string, error) {
	token := Token{
		Message:        message,
...
func VerifyToken(token string, verificationKey *rsa.PublicKey) (*Token, error) {
	...
	if err := VerifySignature(string(decodedMessage), decodedSignature, verificationKey); err != nil {
		return nil, err
	}
